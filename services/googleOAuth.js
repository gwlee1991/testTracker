...
const handleCallback = async (req) => {
    try {
        ...
        const response = await axios.get("https://www.googleapis.com/oauth2/v3/userinfo?profile%20email", {
            headers: {
        ...
        if (!user) {
            user = new User({ googleId: sub, name: name, email: email, isAdmin: false }).save();
        }
        return user;
    } catch (e) {
