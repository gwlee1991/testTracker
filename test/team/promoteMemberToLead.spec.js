...
chai.use(chaiHttp);
const url = "/api/team/members/promote";

const UNAUTHORIZED_REQUEST = "UNAUTHORIZED_REQUEST";

describe("/api/team/members/promote test cases", () => {
  before(async () => {
  ...
  it("should send error if lead is attempted to be promoted", async () => {
    const request = chai.request(app).post(url);
    request.set("Authorization", `Bearer ${this.token}`);
    const res = await request.send({
      teamId: this.team._id,
  ...
  it("should promote member to lead when all is well", async () => {
    const request = chai.request(app).post(url);
    request.set("Authorization", `Bearer ${this.token}`);
    const res = await request.send({
      teamId: this.team._id,
      userId: this.mockUser2._id,
    });

    expect(res.body.leadIds).to.include(this.mockUser2._id.toString());
